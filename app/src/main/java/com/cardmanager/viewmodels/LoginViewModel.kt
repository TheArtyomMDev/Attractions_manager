...
import com.cardmanager.utils.Constants.TAG
...
class LoginViewModel(application: Application, private val auth: FirebaseAuth):
    ...
    private fun validate(email: String, password: String): Boolean {
        ...
        if (email.isEmpty() || !android.util.Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            emailInputError.value = getApplication<Application>().resources.getString(com.cardmanager.R.string.email_input_error)
            valid = false
        } else {
        ...
        if (password.isEmpty() || password.length < 6) {
            passwordInputError.value = getApplication<Application>().resources.getString(com.cardmanager.R.string.password_input_error)
            valid = false
        } else {
